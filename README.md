...
6. 输入 `make -j1 V=s` （-j1 后面是线程数。第一次编译推荐用单线程）即可开始编译你要的固件了。`nohup make -j1 V=s > make.log & tail -f make.log`
...
make -j$(($(nproc) + 1)) V=s
```
`rm -rf lede/ && git clone https://github.com/coolsnowwolf/lede && cd lede && sed -i '$a\src-git kenzo https://github.com/kenzok8/openwrt-packages' feeds.conf.default && sed -i '$a\src-git small https://github.com/kenzok8/small' feeds.conf.default && ./scripts/feeds update -a && ./scripts/feeds install -a && make menuconfig`
